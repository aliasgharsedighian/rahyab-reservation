...
import { ChevronDownIcon, InfoIcon, MessageCircleIcon, StarIcon, UtensilsIcon } from "lucide-react";
...
function FoodReserveTabs({ reserveList }: { reserveList: FoodReserveList | null }) {
  ...
  const renderFood = (food: ReserveFood, group: ReserveFoodGroup, day: ReserveDay) => (
    <article
    ...
    >
      <div className="order-2 flex min-w-0 flex-col gap-3 p-4 sm:order-1">
        <div className="flex min-w-0 flex-col items-start gap-2 sm:flex-row sm:justify-between">
          <h3 className="w-full min-w-0 text-lg font-bold leading-7 text-(--base-black) sm:w-auto">{food.name}</h3>
          <Button onClick={() => handleOpenModal(food.food_id)} variant="outline" className="h-auto max-w-full shrink-0 self-start rounded-full p-0">
            <span className="flex max-w-full items-center gap-1.5 whitespace-nowrap rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[11px] text-zinc-700 sm:text-xs">
              ...
              <span className="font-semibold tabular-nums">{food.feedback_rate ?? "—"}</span>
              <span className="text-zinc-500">({food.feedback_count ?? 0} رأی) - ({food.comment_count ?? 0} نظر)</span>
            </span>
          </Button>
        </div>
        {food.description ? <p className="text-sm leading-6 text-(--secondary-text)">{food.description}</p> : null}
        <span className="text-base font-semibold text-(--base-black)">{food.price.toLocaleString("fa-IR")} تومان</span>
        <FoodReserveInvoice
          {...food}
          day={day.day_name}
        ...
        />
        <Button variant="link" className="h-auto w-fit gap-1.5 p-0 text-(--base-green)" onClick={() => handleOpenModal(food.food_id)}>
      ...
      <div className="order-1 flex min-h-36 items-center justify-center bg-zinc-100 sm:order-2 sm:m-3 sm:aspect-square sm:min-h-0 sm:self-center sm:rounded-xl dark:bg-zinc-800">
        {food.image_url ? (
          ...
          <img className="aspect-[16/10] h-full w-full rounded-xl object-contain sm:aspect-square" src={food.image_url} alt={food.name} loading="lazy" />
        ) : (
        ...
        <div className="flex flex-col gap-4">
          <TabsList className="rtl w-full bg-(--light-green)">
            {reserveList?.weeks.map((week) => (
              <TabsTrigger key={week.range} value={week.range} className="text-[12px] md:text-base">
                {week.is_current_week ? "هفته جاری" : `${week.week_number} هفته بعد`}
              </TabsTrigger>
            ))}
          </TabsList>
          {reserveList?.weeks.map((week) => (
            <TabsContent key={week.range} value={week.range}>
              {week.reservation_message ? (
                <div className="mb-2 flex items-center gap-2 rounded-xl bg-(--light-green) p-4">
                  <InfoIcon className="size-5 text-red-600" /><p className="text-red-600">{week.reservation_message}</p>
                </div>
              ) : null}
                ...
                {week.days.map((day) => {
                  ...
                  return (
                    <section key={day.date} className="flex flex-col gap-5 rounded-2xl border bg-white p-3 shadow-sm md:p-5 dark:bg-zinc-800">
                      <header className="flex items-center gap-3 text-lg"><span className="h-8 w-0.5 bg-(--base-green)" /><strong>{day.day_name}</strong><span>{day.jalali_date}</span></header>
                      {mainGroups.length ? mainGroups.map((group) => (
                        <div key={group.type} className="grid gap-3">
                          {mainGroups.length > 1 ? <h2 className="w-fit rounded-full bg-(--light-green) px-4 py-1.5 font-bold text-(--base-green)">{group.type_fa || fallbackTypeTitle[group.type]}</h2> : null}
                          {group.items.map((food) => renderFood(food, group, day))}
                        </div>
                      )) : <div className="rounded-xl bg-zinc-50 p-5 text-center text-sm text-(--secondary-text)">غذای اصلی برای این روز وجود ندارد</div>}
                      {sideGroups.length ? (
                        <Collapsible
                        ...
                        >
                          <CollapsibleTrigger className="flex w-full items-center justify-between gap-3 p-3 text-right md:p-4">
                            <span>
                              <span className="block font-bold">مخلفات اختیاری</span>
                          ...
                          <CollapsibleContent className="px-3 pb-3 md:px-4 md:pb-4">
                            <div className="grid gap-5 border-t border-(--base-green)/15 pt-4">
                              {sideGroups.map((group) => (
                                <div key={group.type} className="grid gap-3">
                                  <h3 className="text-sm font-bold text-(--base-green)">{group.type_fa || fallbackTypeTitle[group.type]}</h3>
                                  {group.items.map((food) => renderFood(food, group, day))}
                                </div>
                              ))}
                            </div>
